...
namespace TMDbLib.Client
{
    ...
    {
        public Movie GetMovieLocal(string imdbId, string language = "", MovieMethods extraMethods = MovieMethods.Undefined)
        {
            ...
            IRestResponse<Movie> response =  _client.Execute<Movie>(request);
        ...
        public async Task<Movie> GetMovie(int movieId, MovieMethods extraMethods = MovieMethods.Undefined)
        {
            try
            {
                var l= await GetMovie(movieId, DefaultLanguage, extraMethods);
                return l;
            }
            catch (Exception ex)
            {
